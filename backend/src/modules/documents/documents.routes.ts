...
router.get('/', (req, res) => {
  ...
  const rows = businessId
    ? db.prepare(`SELECT d.*, s.original_name, s.size_bytes
      FROM documents d
      JOIN source_files s ON s.id = d.source_file_id
      WHERE d.business_id = ?
      ORDER BY d.uploaded_at DESC`).all(businessId) as Array<Record<string, unknown>>
    : db.prepare(`SELECT d.*, s.original_name, s.size_bytes
      FROM documents d
      JOIN source_files s ON s.id = d.source_file_id
...
router.post('/upload', upload.single('file'), (req, res) => {
  ...
  db.prepare(`INSERT INTO documents
    (id, workspace_id, business_id, source_file_id, file_name, mime_type, notes)
    VALUES (?, ?, ?, ?, ?, ?, ?)`).run(
    documentId,
    ((req.body.businessId as string | undefined)
      ? (db.prepare('SELECT workspace_id FROM businesses WHERE id = ?').get(req.body.businessId as string) as { workspace_id: string } | undefined)?.workspace_id ?? null
      : null),
    (req.body.businessId as string | undefined) ?? null,
    sourceFileId,
    req.file.originalname,
    req.file.mimetype || 'application/octet-stream',
    req.body.notes ?? null
