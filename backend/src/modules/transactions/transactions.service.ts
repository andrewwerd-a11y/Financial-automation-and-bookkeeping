...
import { evaluateTransactionPolicies } from '../policies/policies.service.js';
...
export const createTransaction = (params: {
  ...
  const workspaceId = params.workspaceId ?? (params.businessId
    ? (db.prepare('SELECT workspace_id FROM businesses WHERE id = ?').get(params.businessId) as { workspace_id: string } | undefined)?.workspace_id ?? null
    : null);

  const id = makeId('txn');
  db.prepare(`INSERT INTO transactions
    (id, date, vendor, amount, description_raw, source_type, source_file_id, workspace_id, business_id, connector_id, external_source_id, status,
      category_suggested, business_activity_suggested, confidence_score, review_status, duplicate_status,
      treatment_suggested, treatment_confidence, treatment_reason, accountant_review_flag, mixed_use_flag, excluded_flag, policy_flags_json)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`) 
    .run(
      id,
      params.date,
      ...
      JSON.stringify(policyEvaluation.flags)
    );
