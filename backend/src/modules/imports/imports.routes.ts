...
const parseAndImportFile = (file: Express.Multer.File, opts: { templateId?: string; saveTemplateName?: string; businessId?: string }) => {
  ...
  const jobId = createIngestionJob({
    jobType: 'csv_import',
    sourceFileId,
    metadata: { fileName: file.originalname, templateId: opts.templateId ?? null, businessId: opts.businessId ?? null }
  });
...
router.post('/csv', upload.single('file'), (req, res) => {
  ...
  try {
    const result = parseAndImportFile(req.file, {
      ...
      businessId: (req.body?.businessId as string | undefined) || undefined
    });
...
router.post('/csv/bulk', upload.array('files', 20), (req, res) => {
  ...
  const results = files.map((file) => {
    try {
      return { fileName: file.originalname, ok: true, result: parseAndImportFile(file, { templateId, businessId: (req.body?.businessId as string | undefined) || undefined }) };
    } catch (error) {
